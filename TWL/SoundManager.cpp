...
SoundManager::SoundManager()
{
	...
	// Associate the sound buffers with the sounds
	...
	// When the player is 50 pixels away from the sound, it will be at full volume
	float minDistance = 50.0f;
	// The sound reduces steadily as the player moves away further away 
...
void SoundManager::playFire(Vector2f emitterLocation, Vector2f listenerLocation)
{
	...
	case 1:
		...
		{
			// Play the sound if is not already playing
			m_Fire1Sound.play();
		}
		break;
	case 2:
	...
	// Increment the next sound to play
	m_NextSound++;
	
	// Reset the next sound to play if we have played all 3
	if (m_NextSound > 3)
	{
		m_NextSound = 1;
	}
